...
module.exports = fee = async (client, m, chatUpdate, store) => {
    try {
        ...
        var msgFee = m.message?.extendedTextMessage?.contextInfo?.quotedMessage || null;
        ...
        const DevFee = Array.isArray(sudoUsers) ? sudoUsers : [];
        const Owner = DevFee.map((v) => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender);
        ...
        const context = {
            ...
            pushname, body, budy, totalCommands, args, mime, qmsg, msgFee, botNumber, itsMe,
            packname, generateProfilePicture, groupMetadata, feespeed, mycode, fetchJson, exec,
